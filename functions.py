...
# Helper functions for Carbon Footprint Calculator
...
# Sample input template used to align model input features
sample = {
 'Body Type': 2,
 'Sex': 0,
 'How Often Shower': 1,
...
# Converts user input into numerical format for ML model
def input_preprocessing(data):
    data["Body Type"] = data["Body Type"].map({'underweight':0, 'normal':1, 'overweight':2, 'obese':3})
    data["Sex"] = data["Sex"].map({'female':0, 'male':1})
...
# Calculates contribution of each component to carbon footprint
def calculate_components(model, ss, sample_df):
...
# Creates pie chart of carbon footprint breakdown
def chart(model, scaler, sample_df, prediction):

    p = calculate_components(model, scaler, sample_df)
    ...
    patches, texts = plt.pie(
        ...
        explode=[0.03] * 4,
        labeldistance=0.75,
        colors=["#29ad9f", "#1dc8b8", "#99d9d9", "#b4e3dd"],
        shadow=True,
        textprops={'fontsize': 20, 'weight': 'bold', "color": "#000000ad"}
    )
    ...
    font1 = ImageFont.truetype("./style/ArchivoBlack-Regular.ttf", size=50)
    font2 = ImageFont.truetype("./style/arialuni.ttf", size=50)
    ...
    final_image = Image.open(data2).resize((700, 700))

    data3 = io.BytesIO()
    final_image.save(data3, "PNG")
